-- Avatar storage access rules. Bucket is private (workspace policy blocks
-- public buckets), so reads are granted to signed-in users and the app
-- uses signed URLs.
CREATE POLICY "Avatar images readable by signed-in users" ON storage.objects FOR SELECT TO authenticated

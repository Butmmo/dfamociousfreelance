...
/** The dropdown itself only ever shows this many — anything beyond it lives on the full /notifications page. */
      ...
      {open && (
        ...
        >
          ...
          {loading ? (
            ...
            <>
              {items.slice(0, DROPDOWN_MAX).map((n) => (
                <button
                  key={n.id}
              ...
              {items.length > DROPDOWN_MAX && (
                <Link
                  to="/notifications"
                  onClick={() => setOpen(false)}
                  className="flex items-center justify-center gap-1.5 px-3 py-2.5 border-t border-border text-xs font-semibold text-primary hover:bg-muted transition"
                >
